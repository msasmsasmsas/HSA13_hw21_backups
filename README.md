...
## Notes
- Ensure the `postgres/postgresql.conf` file is mounted in `docker-compose.yml` for WAL support.
- Use the `time` command to measure backup/restore duration (e.g., `real`, `user`, `sys` times on Linux/macOS).
- The `books` table must exist before backups; run the initialization steps if needed.
```

---

### Объяснение
1. **`postgresql.conf`:** Настроен для поддержки WAL, что необходимо для Incremental и Reverse Delta стратегий.  
2. **`README.md`:**  
   - Включает запуск проекта (`docker-compose`), инициализацию таблицы и наполнение данными с помощью `insert_db.py`.  
   - Описаны все четыре стратегии с командами для бэкапа и восстановления, включая фиксацию времени через `time`.  
   - Добавлено сравнение стратегий в таблице и рекомендации по их использованию.  
   - Форматирование соответствует Markdown с заголовками, кодовыми блоками и разделителями.

Если нужно что-то уточнить или адаптировать (например, структуру `docker-compose.yml`), дайте знать!
